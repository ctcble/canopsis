...
def _roundtime(utcdate, periodtime=1, periodtype=T_HOUR, timezone=0):
    """
    ...
    if relativeinterval is not None:
...
def roundTime(date, interval, timezone=0):
    """
...
def getTimeSteps(start, stop, interval, roundtime=True, timezone=0):
    ...
    if mode == 'by_point':
        ...
        for index in xrange(1, len(timeSteps)):
            ...
            aggregation_point = get_aggregation_point(
                points_to_aggregate, agfn, previous_timestamp, fill)
        ...
        if i < len(points):
            ...
            aggregation_point = get_aggregation_point(
                points_to_aggregate, agfn, timeSteps[-1], fill)
