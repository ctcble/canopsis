...
class manager(object):
    ...
    def uncompress_meta_fields(self, meta_data):
        for field in self.fields_map:
            value = meta_data.get(
                self.fields_map[field][0], self.fields_map[field][1])
    ...
    def compress_meta_fields(self, meta_data):
        ...
        for field in self.fields_map:
            meta_data = set_meta(
                meta_data,
                field,
                self.fields_map[field][0],
                self.fields_map[field][1])
    ...
    def get_points(self, _id=None, name=None, tstart=None, tstop=None, raw=False, return_meta=False, add_prev_point=False, add_next_point=False):
        ...
        if add_prev_point or add_next_point:
            ...
            # Add points
            if add_prev_point and prev_index is not None and prev_index >= 0:
            ...
            if add_next_point and next_index is not None and next_index < len(points):
    ...
    def get_point(self, _id=None, ts=None, name=None, return_meta=False):
        ...
        if not ts:
            ...
            (meta, points) = self.get_points(
                _id=_id, tstart=ts, tstop=ts, add_prev_point=True, return_meta=True)
    ...
    def clean(self, _id=None, name=None, timestamp=None):
        ...
        if _id:
            meta = self.find(
                limit=1, mfilter={'_id': _id, 'fts': {'$lt': timestamp}})
            if meta:
                metas = [meta]
            else:
                metas = []
    ...
    def remove(self, _id=None, name=None, purge=True):
        ...
        if len(bin_dcas):
            ...
            self.store.db[self.store.mongo_collection+"_bin.chunks"].remove(
                {'files_id': {'$in': bin_dcas}})
            self.store.db[self.store.mongo_collection+"_bin.files"].remove(
                {'_id': {'$in': bin_dcas}})
        ...
        if len(dcas):
            ...
            if len(dcas) == 1:
                ...
                self.store.remove(
                    mfilter={'_id': {'$in': [dca['_id'] for dca in dcas]}})
    ...
    def showStats(self):
        ...
        if mcount:
            self.logger.info(
                "Size/metric: %.3f KB", ((float(size)/mcount)/1024.0))
