...
class store(object):
    ...
    def push(self, _id, point, meta_data={}, bulk=True):
        ...
        # Calcul push rate
        if self.pushed_values and \
                (self.last_rate_time + self.rate_interval) < now:
        ...
        if bulk and self.pipe_size == 0:
            self.logger.debug(
                "Bulk mode is enabled (rate: %s push/sec)", self.last_rate)
        ...
        # Sync DB if need
        if self.pipe_size and self.redis_sync_interval and \
                (self.last_sync + self.redis_sync_interval) < now:
            self.sync()
